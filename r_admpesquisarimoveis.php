...
if ($query->num_rows < 1) {
    ...
    while ($imovel_disponivel = $query->fetch_array()) {
		...
		<b>Código reduzido:</b> ' . nl2br2($imovel_disponivel[cd_imovel_reduzido]) . '
                ...
                <b>Última Vistoria:</b>' . traduz_data_para_exibir($imovel_disponivel[ultima_vistoria]) . '<br/>
                <b>Observações ou Averbações:</b> ' . nl2br2($imovel_disponivel[observacoesimovel]) . '<br /></fieldset>';
        ?>
        <form name="formselecionaVistoria" method="post" action="adm_solicitarVistoria2.php">

            <input type="hidden" name="idimovel" value="<?php $id = intval($imovel_disponivel[idimovel]); echo $id; ?> "/>
...
<br>
<form action="admpesquisardisponivel.php">
